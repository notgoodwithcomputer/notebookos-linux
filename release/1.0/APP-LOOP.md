...
| 5 | calendar | 2026-08-09 | **DONE** — task 049; **6 defects**, 9 → 12 suites, 157 → **247 checks**, 0 red. TWO DATA-LOSS DEFECTS ON FILE ▸ OPEN: any foreign JSON carrying an `events` list replaced the whole calendar (a playlist, a log, a list of nulls — 3 events → 0, Open reported as SUCCESS), and a dict with `calendars` and no `events` key read silence as "delete them". Plus a shared-store merge that dropped any non-dict record against a promise in the line below it, and TWO i18n defects found by RENDERING in German: the French "today" needed a typographic apostrophe no keyboard makes, and Turkish lost its word in capitals (`str.lower()` is not locale-aware). Recurrence checked and CORRECT — primitives and caller both. 3 new strings (Noon/Midday/Midnight) for the merge. |
| 6 | contacts | 2026-08-10 | **IN PROGRESS** — claimed 00:35, task 050. Baseline 5 suites / 98 checks; now **6 / 123**. A colon in typed text was always read as a label separator, so `http://example.com` was stored as `//example.com` and `3:30 meeting` as `30 meeting`. vCard escaping and the birthday logic MEASURED CORRECT (11111 exhaustive round-trips, zero failures) and recorded as such. |
